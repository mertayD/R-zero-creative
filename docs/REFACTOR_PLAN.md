...
@app.function(
    ...
    secrets=[modal.Secret.from_name("critic-judge-token")],
    timeout=3600 * 24,
)
...
Auth is a shared bearer token (Modal secret), read the same way `ClaudeAgent` reads `PERPLEXITY_API_KEY`. The run's own aux-GPU accounting in `orchestrator.py` (`n_training_gpus`, `aux_gpu_id`) is untouched — this judge never consumes a GPU on the training node, deployed or idle.
...
- Do: new `modal_app.py` function/image serving `AQuarterMile/WritingBench-Critic-Model-Qwen-7B` via `vllm.entrypoints.openai.api_server` behind `@modal.web_server`, `min_containers=0` + a tuned `scaledown_window`, bearer-token auth via a Modal secret. `modal deploy modal_app.py`, once, separately from any training run. Add `orchestrator.py::wait_for_sft_critic_judge(url, ...)` — a `solver_server`-style poll against the deployed URL — called once at the start of `run_coevolve` when `cfg.judge.type == "sft-critic"`, so a cold start (if any) happens before Phase A rather than stalling the first judge call.
...
- Accept: `curl -H "Authorization: Bearer $TOKEN" https://.../v1/chat/completions -d '...'` returns a completion; `modal container list` shows 0 containers for the app some time after a run finishes; `orchestrator.py::n_training_gpus`/`aux_gpu_id` are unaffected by this change.
...
- `judge.type: sft-critic` dispatch landed in both reward callers' `_get_agent()` exactly per the §6.3 dict-dispatch sketch (`agent` and `scorer_cls` chosen independently).
- `CriticServerAgent` additionally supports an optional bearer token (`CRITIC_JUDGE_API_KEY`, sent only if set) against vLLM's own `--api-key` flag — not required, since a self-hosted deployment may run without auth (e.g. local dev), unlike `ClaudeAgent`'s `PERPLEXITY_API_KEY` which is required because the external API mandates it.
...
- All 170 repo tests pass (`(venv) pytest tests/ -q`), including the 5 new/extended files covering: config validation, prompt fidelity (pinned sha256, cross-checked against the already-vendored `prompt.py`), the HTTP client's retry/backoff/error semantics, the per-criterion request/response contract, dispatch wiring, and the orchestrator's warm-up poll (success / cold-start-retry / timeout / called-once-per-run / not-called-for-other-judge-types).
