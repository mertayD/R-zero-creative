# Vendored verbatim from https://github.com/X-PLUG/WritingBench/blob/main/prompt.py
# (Apache-2.0). Do NOT edit: keeping this byte-identical to upstream guarantees
# scores are directly comparable to the WritingBench leaderboard.
#
# This is a dedicated copy for the training-loop per-criterion judge path
# (evaluator/critic_server.py::CriticServerAgent + batch_eval_agent.py::
# PerCriterionEvalAgent, judge.type=sft-critic — see docs/REFACTOR_PLAN.md
# §6.4) — deliberately not imported from prompt.py, which evaluate_benchmark.py
# (the standalone WritingBench-benchmark CLI script) owns and is free to
# change for reasons unrelated to training. Both files currently hold
# identical content; that is expected, not a bug to deduplicate away.
